...
exports.smsg = (conn, m, store) => {
    ...
    conn.presence = async (type = 1) => {
        ...
        switch(type){
            case 0:{ update = 'unavailable' } break
            case 1:{ update = 'available' } break
            case 2:{ update = 'composing' } break
            case 3:{ update = 'recording' } break
            case 4:{ update = 'paused' } break
        }
...
exports.delays = (tod) => {
    ...
    return new Promise(resolve => {
        setTimeout(() => {
            resolve();
        }, gay * 1000); // Waktu dalam milidetik (3000 ms = 3 detik)
    });
}
